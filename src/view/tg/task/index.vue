...
export default defineComponent({
  ...
  setup() {
    ...
    // 判断是否为子路由，只有特定路径才显示子路由内容
    const isSubRoute = computed(() => {
      return route.path.includes('/send-message') || 
             route.path.includes('/join-group') || 
             route.path.includes('/add-contact') || 
             route.path.includes('/extract-members') || 
             route.path.includes('/check-account')
    })
