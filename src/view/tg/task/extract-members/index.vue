<template>
  <div class="extract-members-task-container">
    <h2>提取成员任务</h2>
    ...
    <el-card class="task-list">
      ...
      >
        ...
        <el-table-column prop="groupInfo" label="目标群组" min-width="140" />
        <el-table-column prop="extractedCount" label="提取数量" width="100" />
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="scope">
            ...
            <el-button type="success" size="small" @click="downloadData(scope.row)" v-if="scope.row.status === 'completed'">下载</el-button>
...
export default defineComponent({
  ...
  setup() {
      ...
      try {
        ...
        setTimeout(() => {
          taskList.value = Array.from({ length: 10 }, (_, index) => ({
            taskId: 'TSK' + (40001 + index),
            taskName: '提取成员任务-' + (index + 1),
            accountInfo: '+1 ' + Math.floor(1000000000 + Math.random() * 9000000000) + ' (user' + (1000 + index) + ')',
            groupInfo: 'Group' + (100 + Math.floor(Math.random() * 50)),
            extractedCount: Math.floor(100 + Math.random() * 500),
    ...
    // 创建任务
    const createTask = () => {
      ElMessageBox.alert('创建提取成员任务功能开发中...', '提示')
    }
    
    // 查看任务详情
    const viewTaskDetail = (task) => {
      ElMessageBox.alert(`任务ID: ${task.taskId}<br>任务名称: ${task.taskName}<br>账号: ${task.accountInfo}<br>群组: ${task.groupInfo}<br>状态: ${getStatusText(task.status)}`, '任务详情', {
    ...
    const downloadData = (task) => {
      ElMessage.success(`正在下载${task.taskName}的数据，共${task.extractedCount}条记录`)
    }
...
<style scoped>
.extract-members-task-container {
  padding: 20px;
}
