...
export const useRouterStore = defineStore('router', () => {
  ...
  const SetAsyncRouter = async () => {
    ...
    if (!asyncRouter || asyncRouter.length === 0) {
      // 如果菜单为空，从 menu.js 中直接导入TG菜单
      const { tgMenus } = await import('@/view/tg/menu')
      asyncRouter.push(...tgMenus)
      console.log('从 menu.js 加载 TG 菜单:', tgMenus)
    } else {
      console.log('从 API 加载菜单:', asyncRouter)
    }
